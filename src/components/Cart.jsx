    ...
    <Layout>
      <div className="text-center py-10">
        ...
        <div className="flex flex-wrap items-center px-4 sm:px-6 md:px-8 lg:px-12 space-x-1 text-xs sm:text-sm text-color font-poppins">
          <HiOutlineHome onClick={() => navigate("/")} className="cursor-pointer" />
          <TbMathGreater />
          <span onClick={() => navigate("/")} className="cursor-pointer">
            Home
          </span>
          <TbMathGreater />
          <span onClick={() => navigate("/learn-more")} className="cursor-pointer">
            Collection
          </span>
          <TbMathGreater />
        ...
        {/* Cart Items (Reversed Order) */}
        {cartItems.slice().reverse().map((item, index) => {
          const originalIndex = cartItems.length - 1 - index;
              ...
              <div className="flex flex-wrap sm:flex-nowrap gap-4 lg:gap-8 items-start lg:items-center justify-between lg:justify-end mt-4 lg:mt-0">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateQuantity(originalIndex, parseInt(e.target.value))
                    }
                  >
                    {[1, 2, 3, 4].map((qty) => (
